...
This page is about **quota providers** — the twenty-six cases Pulse can draw a ring for. Token spend readers are a different, overlapping catalogue of agents that left records on this Mac, most of which Pulse draws no ring for; they and their stores are documented in [`../token-spend.md`](../token-spend.md) and [`../token-spend-sources.md`](../token-spend-sources.md), not here. Do not add a spend reader to this directory.
...
Ollama Cloud, Xiaomi's two rows, Qoder and StepFun are the five read from a **browser session** rather than a key or another tool's files; they share [`BrowserCookies.swift`](../../Sources/Pulse/Auth/BrowserCookies.swift) and nothing else, because what counts as a session differs per site and a shared filter would forward whichever cookie any one of them adds next. Qoder's filter is the one deny list among them, because its session cookie has no published name ([qoder.md](qoder.md)).

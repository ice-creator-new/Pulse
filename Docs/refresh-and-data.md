...
**One timer, but not one cadence.** Under `.automatic` each provider has its own interval and the timer is set for whichever is due soonest; a pass then asks only the accounts that are actually due (`UsageStore.providersToAsk`, paced from `askedAt` — *asked*, not answered, or a provider that refuses every time reads as permanently due and spins the loop). Everything not asked keeps the reading it has, because the commit loop is gated on the same set. A **fixed** interval chosen in Settings applies to everything equally: somebody who picked five minutes meant five minutes. **Added accounts are not paced this way, deliberately**: every pass asks all of them, whichever primary woke it (the `extras` comment in `UsageStore.refresh`). A PR giving each its own cadence (#62) was declined on that basis; revisit it only on a reported problem.
